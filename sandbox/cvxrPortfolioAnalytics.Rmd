...
In this section, we use the CRSP data set to generate different portfolios and show their performance by plotting cumulative gross returns and efficient frontiers.

The entire back-testing part may be slow, occupying most of the time in this Vignette. Therefore, we print out the time for readers' reference. My computer model is MacBook Air with M2, 8-Core CPU, 8-Core GPU and 16-core Neural Engine. The backtesting in Sections 9.1 and Section 9.2 takes about 3 minutes each to run, and the average running time for generating efficient frontiers in Section 9.3 is about 30 seconds.

The CRSP data set is an xts object that contains the daily returns of stocks from 1993-01 to 2015-12^[CRSP: Center for Research in Security Prices, LLC.]. We select the top 30 small cap stocks based on the market capitalization, and use this data set to show more meaningful and interesting results in this section.
```{r}
...
In this example, we use daily return of all the CRSP 30 stocks to generate a comparative backtesting among Global Minimum Variance (GMV), Global Minimum ES (GMES) and Global Minimum EQS (GMEQS) portfolio. The strategy is to rebalance the portfolio at the end of each month with a rolling window of 500 days, and the performance of backtesting could be shown as a plot of cumulative returns and a plot of drawdown.
```{r, echo=FALSE}
# test run time
...
We generate efficient frontiers with mean-StdDev, mean-ES and mean-EQS portfolios using 30 stocks from CRSP data set. For illustrative purposes, we use the last five years
