...
# 9 Comparative Performance of GMV, GMES, GMEQS Portfolios
...
In this section, we use the CRSP data set to generate GMV, ES and EQS portfolios and show their performance by plotting cumulative returns and efficient frontiers.
...
The entire back-testing part may be slow, occupying most of the time in this Vignette. Therefore, we print out the time for readers' reference. My computer model is MacBook Air with M2, 8-Core CPU, 8-Core GPU and 16-core Neural Engine.
```{r}
...
cat("Section 9.1 cost", format(runningtime1))
```
...
cat("Section 9.2 cost", format(runningtime2))
```
